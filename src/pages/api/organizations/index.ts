...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.artillery_calculation?.length > 0) {
      const create_artillery_calculation = body.artillery_calculation;
      body.artillery_calculation = {
        create: create_artillery_calculation,
      };
    } else {
      delete body.artillery_calculation;
    }
